...
Start:
	...
	InputInt(mark, 0, 10, "Введите нужный средний балл\n", "\n");
	cout << "\n" << "Cтуденты группы " << input << " со средним баллом выше " << mark << ":\n\n";
	for (int i = 0; i < n; i++) if (students[i].group == input && students[i].averageMark > mark) ShowStudentInfo(students[i], i);
	goto Start;
}
