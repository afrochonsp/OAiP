...
Start:
	...
	cout << "\n" << "Cтуденты группы " << input << " с фамилией на буквы В, Г и Д:\n\n";
	for (int i = 0; i < n; i++)
		if (students[i].group == input && (students[i].name[0] == 'В' || students[i].name[0] == 'Г' || students[i].name[0] == 'Д')) ShowStudentInfo(students[i], i);
	goto Start;
}
