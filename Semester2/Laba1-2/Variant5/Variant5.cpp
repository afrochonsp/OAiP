...
Start:
	...
	cout << "\n" << "Студенты, имеющие оценку 4 или 5 по физике и оценку больше 8 по остальным предметам\n\n";
	for (int i = 0; i < n; i++)
		if ((students[i].physics == 4 || students[i].physics == 5) && students[i].math > 8 && students[i].informatics > 8 && students[i].chem > 8)
			ShowStudentInfo(students[i], i);
	goto Start;
}
