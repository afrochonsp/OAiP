...
Start:
	int n, input;
	float averageMark = 0, groupAverageMark = 0;
	Student *students;
	...
	for (int i = 0; i < n; i++) if(students[i].group == input) groupAverageMark += students[i].averageMark / n;
	cout << "\n" << "Средний балл студентов группы " << input << ": " << groupAverageMark << "\n\n";
	cout << "\n" << "Студенты группы "  << input << " со средним баллом больше общего среднего балла:\n\n";
	for (int i = 0; i < n; i++) averageMark += students[i].averageMark / n;
	for (int i = 0, j = 0; i < n; i++)
	{
		if (students[i].averageMark > averageMark && students[i].group == input)
		{
...
void InputInt(int &n, int minVlaue, int maxValue, string text1, string text2)
{
	cout << text1;
