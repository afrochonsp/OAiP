...
Start:
	...
	case 2:
	{
		if (!list->begin) break;
		AddToBegin(list, 0);
		cout << list->begin->value;
		ListElement *preMinElement = list->begin, *preMaxElement = list->begin, *begin = list->begin;
		for (ListElement *tempPointer = list->begin; tempPointer->next; tempPointer = tempPointer->next)
		{
			if (tempPointer->next->value < preMinElement->next->value) preMinElement = tempPointer;
			if (tempPointer->next->value > preMaxElement->next->value) preMaxElement = tempPointer;
		}
		if (preMinElement->next == preMaxElement || preMaxElement->next == preMinElement)
		{
			if (preMinElement->next == preMaxElement)
			{
				preMinElement->next = preMaxElement->next;
				preMaxElement->next = preMinElement->next->next;
				preMinElement->next->next = preMaxElement;
			}
			else
			{
				preMaxElement->next = preMinElement->next;
				preMinElement->next = preMaxElement->next->next;
				preMaxElement->next->next = preMinElement;
			}
		}
		else
		{
			ListElement *MinElement = preMinElement->next, *MaxElementNext = preMaxElement->next->next;
			preMinElement->next = preMaxElement->next;
			preMaxElement->next->next = MinElement->next;
			preMaxElement->next = MinElement;
			MinElement->next = MaxElementNext;
		}
		Delete(begin, list);
		break;
	}
