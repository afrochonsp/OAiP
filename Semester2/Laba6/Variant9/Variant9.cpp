...
Start:
	...
	case 2:
	{
		if (!list->begin) break;
		int count = 0;
		ListElement *minElement = list->begin, *maxElement = list->begin, *lastPointer = list->begin;
		for (ListElement *tempPointer = list->begin->next; tempPointer; tempPointer = tempPointer->next)
		{
			...
			if (minElement == tempPointer || maxElement == tempPointer) lastPointer = tempPointer;
		}
		AddToEnd(list, 0);
		for (ListElement *tempPointer = (lastPointer == minElement ? maxElement : minElement)->next->next; tempPointer->prev != lastPointer; tempPointer = tempPointer->next)
		{
			Delete(tempPointer->prev, list);
			count++;
		}
		Delete(list->end, list);
		cout << "Количество элементов между минимальным и максимальным элементами: " << count << "\n";
		break;
	}
	case 3:
