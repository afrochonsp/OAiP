...
Start:
	...
	case 2:
	{
		for (ListElement *tempPointer = list->begin; tempPointer && tempPointer->next; tempPointer = tempPointer->next)
		{
			if (!tempPointer->next->next)
			{
				ListElement *DownElement = tempPointer->next;
				ListElement *DownElementNext = DownElement->next;
				if (DownElement == list->begin->next) DownElement->next = list->begin;
				else
				{
					DownElement->next = list->begin->next;
					tempPointer->next = list->begin;
				}
				list->begin->next = DownElementNext;
				list->begin = DownElement;
			}
		}
		break;
	}
