...
Start:
	...
	{
		if (!list->begin) break;
		float averageValue = list->begin->value;
		int count = 0;
		for (ListElement *tempPointer = list->begin->next; tempPointer; tempPointer = tempPointer->next) averageValue = (averageValue + tempPointer->value) / 2;
		for (ListElement *tempPointer = list->begin; tempPointer; tempPointer = tempPointer->next) if (tempPointer->value % 2 == 0) tempPointer->value = (int)averageValue;
		break;
	}
	case 3:
