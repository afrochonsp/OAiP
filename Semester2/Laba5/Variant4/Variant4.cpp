...
Start:
	...
	case 2:
	{
		for (Stack *tempPointer = stack; tempPointer && tempPointer->next; tempPointer = tempPointer->next)
		{
			if (!tempPointer->next->next)
			{
				Stack *DownElement = tempPointer->next;
				Stack *DownElementNext = DownElement->next;
				if (DownElement == stack->next) DownElement->next = stack;
				else
				{
					DownElement->next = stack->next;
					tempPointer->next = stack;
				}
				stack->next = DownElementNext;
				stack = DownElement;
			}
		}
		break;
