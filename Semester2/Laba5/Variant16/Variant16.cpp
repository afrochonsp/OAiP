...
Start:
	...
	{
		if (!stack) break;
		float averageValue = stack->value;
		int count = 0;
		for (Stack *tempPointer = stack->next; tempPointer; tempPointer = tempPointer->next) averageValue = (averageValue + tempPointer->value) / 2;
		for (Stack *tempPointer = stack; tempPointer; tempPointer = tempPointer->next) if (tempPointer->value % 2 == 0) tempPointer->value = (int)averageValue;
		break;
	}
	case 3:
