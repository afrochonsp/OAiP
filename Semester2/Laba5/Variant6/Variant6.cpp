...
Start:
	...
	case 2:
	{
		if (!stack) break;
		Add(stack, 0);
		Stack *preMinElement = stack, *preMaxElement = stack, *top = stack;
		for (Stack *tempPointer = stack; tempPointer->next; tempPointer = tempPointer->next)
		{
		...
		{
			Stack *MinElement = preMinElement->next, *MaxElementNext = preMaxElement->next->next;
			preMinElement->next = preMaxElement->next;
