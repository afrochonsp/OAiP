...
Node *&GetMinNode(Node *&, Node *** = NULL);
...
Start:
	...
	case 2:
	{
		...
		float average = 0;
		GetNodes(root, nodes);
		for (auto node : nodes) average += node->key;
		average /= nodes.size();
		if (abs(maxNode->key - average) < abs(minNode->key - average)) cout << maxNode->info << "\n";
		else cout << minNode->info << "\n";
		break;
	}
	case 3:
...
Node *&GetMinNode(Node *&root, Node ***prev)
{
	Node **p = &root;
	while ((*p)->left)
	{
