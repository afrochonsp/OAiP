...
Start:
	...
	case 1:
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < n; j++)
			{
				if (tables[i].name == tables[j].name && tables[i].address == tables[j].address && i != j)
				{
					for (int k = j; k < n - 1; k++) tables[k] = tables[k + 1];
					n--;
				}
			}
		}
	...
	case 3:
		...
		break;
	case 4:
		InputDate(key, "Введите нужную дату:\n");
		cout << "\n";
...
void ChangeTableInfo(Table &table)
{
	...
	cout << "Домашний адрес: ";
	getline(cin, table.address);
	InputDate(table.date, "\nДата учёта:\n");
	cout << "\n";
}
...
void ShowTableInfo(Table table)
{
	...
	cout << "ФИО: " << table.name << "\n";
	cout << "Домашний адрес: " << table.address << "\n";
	ShowDate(table.date, "Дата учёта: ");
	cout << "\n\n";
}
