...
Start:
	...
	{
		int contained = -1;
		for (int j = 0; j < c; j++) if (tables[i].name == groups[j]) contained = j;
		if (contained == -1) groups[c++] = tables[i].name;
	}
	...
	case 1:
		for (int i = 0; i < c; i++)
		{
			for (int j = 0; j < n; j++)
			{
				...
				localtime_s(&date1, &time);
				localtime_s(&date2, &tables[j].date);
				if (date1.tm_mday == date2.tm_mday && date1.tm_mon == date2.tm_mon &&
					date1.tm_year == date2.tm_year && groups[i] == tables[j].name) ShowTableInfo(tables[j]);
			}
		}
		break;
	case 2:
	...
	case 3:
		...
		break;
	case 4:
		InputDate(key, "Нужная дата приёмки в ремонт:\n");
		cout << "\n";
...
{
	cout << "Наименование группы изделий: ";
	getline(cin, table.name);
	cout << "Марка изделия: ";
	getline(cin, table.mark);
	InputDate(table.date, "Дата приёмки в ремонт:\n");
	InputNumber<bool>(table.readiness, "Состояние готовности заказа: ");
	cout << "\n";
}

void ShowTableInfo(Table table)
{
	cout << "Наименование группы изделий: " << table.name << "\n";
	cout << "Марка изделия: " << table.mark << "\n";
	ShowDate(table.date, "Дата приёмки в ремонт: ");
	cout << "\n";
	cout << "Состояние готовности заказа: " << table.readiness << "\n\n";
}
