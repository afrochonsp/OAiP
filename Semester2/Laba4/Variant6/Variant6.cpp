...
template<typename T = int>
void SelectionSort(Table[], int, T Table::* = &Table::salary);
template<typename T = int>
void QuickSort(Table[], int, int, T Table::* = &Table::salary);
template<typename T = int>
int LinearSearch(Table[], int, T, T Table::* = &Table::salary);
template<typename T = int>
int BinarySearch(Table[], int, int, T, T Table::* = &Table::salary);
...
Start:
	...
	{ "Сортировка прямым выбором", "Быстрая сортировка", "Линейный поиск", "Двоичный поиск" } };
...
void ChangeTableInfo(Table &table)
{
	...
	InputNumber(table.number, "Табельный номер: ", 1);
	InputNumber(table.hours, "Количество проработанных часов за месяц: ");
	InputNumber(table.tariff, "Почасовой тариф: ");
	table.salary = (fmin(table.hours, 144) + fmax(table.hours - 144, 0)) * table.tariff * 0.88;
	cout << "Зарплата с налогом: " << table.salary << "\n\n";
}

void ShowTableInfo(Table table)
{
	cout << "ФИО: " << table.name << "\n";
	cout << "Табельный номер: " << table.number << "\n";
	cout << "Количество проработанных часов за месяц: " << table.hours << "\n";
	cout << "Почасовой тариф: " << table.tariff << "\n";
	cout << "Зарплата с налогом: " << table.salary << "\n\n";
}
