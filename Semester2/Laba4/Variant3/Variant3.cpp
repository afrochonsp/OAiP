...
void SelectionSort(Table[], int, T Table::* = &Table::profit);
template<typename T = float>
void QuickSort(Table[], int, int, T Table::* = &Table::profit);
template<typename T = float>
int LinearSearch(Table[], int, T, T Table::* = &Table::profit);
template<typename T = float>
int BinarySearch(Table[], int, int, T, T Table::* = &Table::profit);
...
Start:
	...
	{
	case 1:
		QuickSort(tables, 0, n - 1, &Table::averageMark);
		for (int i = n - 1; i >= 0; i--)
		{
			if (tables[i].profit < 750 && n > 1)
			{
				Table temp = tables[i];
				for (int j = i; j > 0; j--) tables[j] = tables[j - 1];
				tables[0] = temp;
			}
		}
		ShowTablesInfo(tables, n);
	...
	case 3:
		...
		break;
	case 4:
		InputNumber<float>(key, "Введите нужный доход:\n", 0);
		cout << "\n";
...
void ChangeTableInfo(Table &table)
{
	...
	InputNumber(table.group, "Группа: ", 1);
	InputNumber<float>(table.averageMark, "Средний балл: ");
	InputNumber<float>(table.profit, "Доход на каждого члена семьи: ");
	cout << "\n";
}
...
{
	cout << "ФИО: " << table.name << "\n";
	cout << "Группа: " << table.group << "\n";
	cout << "Средний балл: " << table.averageMark << "\n";
	cout << "Доход на каждого члена семьи: " << table.profit << "\n\n";
}
