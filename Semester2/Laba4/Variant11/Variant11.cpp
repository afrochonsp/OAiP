...
Start:
	...
	case 1:
		...
		for (int i = 0; i < n; i++)
			if ((tables[i].marks[0] + tables[i].marks[1] + tables[i].marks[2] + tables[i].marks[3]) / 3.0 >=
				8.5 && tables[i].address == "Минск") ShowTableInfo(tables[i]);
		break;
	case 2:
...
void ChangeTableInfo(Table &table)
{
	...
	InputNumber(table.number, "Номер группы: ");
	cout << "Адрес: ";
...
{
	cout << "ФИО: " << table.name << "\n";
	cout << "Номер группы: " << table.number << "\n";
	cout << "Адрес: " << table.address << "\n";
