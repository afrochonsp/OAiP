...
void SelectionSort(Table[], int, T Table::* = &Table::time);
template<typename T = time_t>
void QuickSort(Table[], int, int, T Table::* = &Table::time);
template<typename T = time_t>
int LinearSearch(Table[], int, T, T Table::* = &Table::time);
template<typename T = time_t>
int BinarySearch(Table[], int, int, T, T Table::* = &Table::time);
...
Start:
	...
	case 1:
		...
		for (int i = 0; i < n; i++) if (tables[i].destination == key) ShowTableInfo(tables[i]);
		break;
	case 2:
	...
	case 3:
		...
		break;
	case 4:
		InputTime(key2, "Введите нужное время прибытия:\n");
		cout << "\n";
		result = LinearSearch(tables, n, key2);
	...
	case 5:
		...
		InputTime(key2, "Введите нужное время прибытия:\n");
		cout << "\n";
		result = BinarySearch(tables, 0, n, key2);
		if (result >= 0) ShowTableInfo(tables[result]);
		else cout << "Элемент не найден\n\n";
		break;
	}
...
{
	InputNumber(table.number, "Номер рейса: ", 1);
	cout << "Тип самолёта: ";
	getline(cin, table.type);
	cout << "Пункт назначения: ";
	getline(cin, table.destination);
	InputTime(table.time, "Время вылета:\n");
	cout << "\n";
}

void ShowTableInfo(Table table)
{
	cout << "Номер рейса: " << table.number << "\n";
	cout << "Тип самолёта: " << table.destination << "\n";
	cout << "Пункт назначения: " << table.destination << "\n";
	ShowTime(table.time, "Время вылета: ");
	cout << "\n\n";
}
